// Interactive logic for the "Better access to education" article.
// Ported from the original standalone index.js. Loads after the ArcGIS Maps SDK
// (global `require`) and D3 (global `d3`) have been defined.
(function start() {
  if (!(window.require && window.d3)) {
  ...
  window.require(["esri/WebScene", "esri/views/SceneView", "esri/Color", "esri/widgets/Home"], (
    ...
    function createHistogram(data, container) {
      ...
      button2020.addEventListener("click", () => {
        svg.select(".g2020").selectAll("rect").transition().duration(500).style("opacity", 0.9);
        svg.select(".g1990").selectAll("rect").transition().duration(500).style("opacity", 0.2);
        button2020.classList.add("selected");
        button1990.classList.remove("selected");
      });
      button1990.addEventListener("click", () => {
        svg.select(".g1990").selectAll("rect").transition().duration(500).style("opacity", 0.9);
        svg.select(".g2020").selectAll("rect").transition().duration(500).style("opacity", 0.2);
        button1990.classList.add("selected");
        button2020.classList.remove("selected");
      });
    }
